...
{{- if $default }}
  ...
  {{- $_ := include "annuums-deployment.containerSecurityContext" (dict "path" "defaultContainerSecurityContext" "securityContext" $default) }}
{{- end }}
...
{{- if hasKey $ctx "capabilities" -}}
  {{- include "annuums-deployment.validateCapabilities" (dict "path" $path "capabilities" $ctx.capabilities) -}}
{{- end -}}
{{- if hasKey $ctx "seccompProfile" -}}
  {{- include "annuums-deployment.validateSeccompProfile" (dict "path" $path "seccompProfile" $ctx.seccompProfile) -}}
{{- end -}}
securityContext:
  {{- if hasKey $ctx "runAsNonRoot" }}
  runAsNonRoot: {{ $ctx.runAsNonRoot }}
...
*/}}
{{- define "annuums-deployment.effectiveContainerSecurityContext" -}}
{{- $name := .name -}}
{{- $effective := dict -}}
...
{{- end -}}
{{- if $effective -}}
{{- include "annuums-deployment.containerSecurityContext" (dict "name" $name "securityContext" $effective) -}}
{{- end -}}
{{- end -}}
...
*/}}
{{- define "annuums-deployment.emptyDirVolume" -}}
{{- $name := .name -}}
